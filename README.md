...
**1.0.3** 新增：桌面端支持从软件外拖入图片和文本（拖文件或拖选中的文字都行）。

修复（构建号 4）：插入的图片会盖住上下两行文字；在资源管理器里复制的图片粘不进来。

修复（构建号 5）：上面那条「资源管理器里复制的图片」其实没修好。原生侧已经把文件路径
传过来了，是客户端自己在判断格式之前就先要求必须有图片字节，把这条路直接挡掉了。
补了三条测试覆盖文件路径这条分支。

**1.0.2** 修掉插入图片后显示成「OBJ」方块的问题；桌面端支持从剪切板粘贴图片。

**1.0.4** 换目录的入口补到看得见的地方：列表里右键笔记可以「移动到…」，
编辑页的菜单里也加了一项。原来只有长按笔记这一个入口——触屏上没问题，
但用鼠标长按（按住不放）不是个自然的动作，等于把功能藏起来了。
...
版本号写在 `pubspec.yaml` 里，格式是 `1.0.3+4`。加号前面是给人看的版本，
加号后面是给系统看的构建号。叠法：

- 加了新功能：两个都加一，`1.0.3+4` → `1.0.4+5`
- 只修 bug 或小改动：只加构建号，`1.0.3+4` → `1.0.3+5`

构建号无论如何都要加，因为 Android 靠它判断"这是不是新版本"，数字不变的话
新打的包装不上去。每次改了什么，记在下面这个更新日志里。
