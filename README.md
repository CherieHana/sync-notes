...
写在 `pubspec.yaml` 里，格式是 `1.4.0+14`。加号前面是给人看的版本号，加号后面
是给系统看的构建号，两个都往上涨，不会倒退。

- **加功能**（多了一个能干的事）：前面的版本号往上顶一位，构建号跟着加一。
  比如 `1.3.0+12` → `1.4.0+13`。
- **修 bug、小改动**：前面的版本号一个字都不动，只加末尾的构建号。
  比如 `1.4.0+14` → `1.4.0+15`。

构建号无论如何都要加，一次不落。Android 就是靠它判断"这是不是新版本"，数字不变
的话新打的包装不上去，会直接提示安装失败。

顺带说一句：加号前面的版本号是"两个都加一"的那位——`1.3.0` 到 `1.4.0` 只动了
中间那位。前面那一位（大版本）留到不兼容的大改才动。
