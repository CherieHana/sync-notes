...
写在 `pubspec.yaml` 里，格式是 `1.0.4+6`。加号前面是给人看的版本，加号后面是
给系统看的构建号。

- 加了新功能：两个都加一，`1.0.4+6` → `1.0.5+7`
- 只修 bug 或小改动：只加构建号，`1.0.4+6` → `1.0.4+7`

构建号无论如何都要加。Android 靠它判断"这是不是新版本"，数字不变的话新打的包
装不上去。
...
85 个测试，另外 1 个真实后端联调测试默认跳过。同步引擎用内存版的本地库和远端来测，
